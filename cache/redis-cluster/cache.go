...
import (
	...
	"github.com/delanri/commonutil/cache"
	"log"
	"sync"
	"time"
)
...
func New(option *Option) (cache.Cache, error) {
	...
	return &redisClusterClient{r: client, channels: make(map[string]cache.PubSub)}, nil
}
...
func (c *redisClusterClient) MGet(key []string) ([]interface{}, error) {
	if err := check(c); err != nil {
		return nil, err
	}
...
func (c *redisClusterClient) Subscribe(channel string) (cache.PubSub, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
